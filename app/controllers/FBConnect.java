...
public class FBConnect extends Controller
{
  ...
  public static void callback()
  {
    ...
    Map<String , Object> extraMap = new HashMap<String , Object>(params.allSimple()); 
    extraMap.remove("body");
    extraMap.remove("code");
    extraMap.remove("error");
    ...
    if (error != null)
    {
      ...
      landUrl += "error_reason=" 
        + WS.encode(params.get("error_reason")) 
        + "&error_description=" 
        + WS.encode(params.get("error_description")) 
        + "&error=" + WS.encode(params.get("error"));
      redirect(landUrl);
    }
    if (code != null)
    {
      Logger.trace("extraMap = %s" , extraMap);
      String authUrl = fbsession.getAuthUrl(code, extraMap);
      Logger.trace("authUrl = %s", authUrl);
    ...
    StringBuffer sb = new StringBuffer(fbsession.getLandUrl());
    if (extraMap.size() > 0)
    {
      sb.append("?");
      for(String key : extraMap.keySet())
        sb.append(key+"="+extraMap.get(key)+"&");
    }
    Logger.trace("redirect to %s", sb.toString());
